...
m=116; Iz=13.1;  %%%  质量  转动惯量
X_udot=-167.6; Y_vdot=-477.2; N_rdot=-15.9;   %%%   附加质量系数
Xu=26.9; Yv=35.8; Nr=3.5;   %%线性水动力阻尼系数
Du=241.3; Dv=503.8; Dr=76.9;  %%%%非线性水动力阻尼系数

Mx=m-X_udot; My=m-Y_vdot; Mpsi=Iz-N_rdot;    %% 等效质量
coef = [m; Iz; X_udot; Y_vdot; N_rdot; Xu; Yv; Nr; Du; Dv; Dr];  %%% 综合系数向量

% Initialize MPC parameters
N = 8;   %% 预测步数
Tstep = 400;  %% 总仿真步数
Tstep2 = Tstep-N;  %% 用于主循环的仿真步数

X0 = [0;1;0;0;0;0]; %% 初始状态向量
nx = length(X0);    %% 状态向量的维度
nu = 3;             %% 控制输入向量的维度

Xall = zeros(nx,Tstep+1);  %% 所有时刻的状态
Uall = zeros(nu,Tstep);    %% 所有时刻的控制输入
eta_Ref_all = zeros(3,Tstep);   %% 存储所有时刻的路径参考
Xall2 = zeros(nx,Tstep2+1);     %% 存储主循环使用的状态  （也就是当前时刻的状态）
Uall2 = zeros(nu,Tstep2);       %% 存储主循环使用的控制输入  （也就是当前时刻的控制输入）
Xall(:,1) = X0;                 %% 初始化第一个时刻的状态
Xall2(:,1) = X0;                %% 初始化主循环的第一个时刻的状态
U0 = Uall2(:,1:N);              %% 初始化控制输入序列

% the control input bound
Fu_max = 2000; Fv_max = 2000; Fr_max = 1000;  %%%  每个方向的推力或扭矩
Umax0 = [Fu_max;Fv_max;Fr_max];               %%%  控制输入上限
Umin0 = [-Fu_max;-Fv_max;-Fr_max];            %%%  控制输入下限
for i = 1:1:N
...
Q = 0.4 * eye(6);   %%% 状态误差加权矩阵 
R = 0.01 * eye(3);  %%% 控制输入误差加权矩阵
Qf = 0.5 * eye(6);  %%% 终端状态误差加权矩阵
weights = {Q, R, Qf};
K1 = 0.5 * eye(3); K2 = -1 * eye(3); 
K = [K1 K2];        %%% K 用于构造可行的控制输入 u0,K将误差转换为控制输入的一部分，作为下一次优化的初始猜测。
disturbance=[0;0;0];%%% 外部干扰向量
ndof = 3;           %%% 自由度
